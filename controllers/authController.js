...
exports.login = catchAsync( async (req, res, next) => {
    ...
    //1)We need to check if email and password actually exist
    if (!email || !password) {
...
exports.forgotPassword =  catchAsync ( async (req, res, next) => {
    ...
    and passwordConfrim to: ${resetURL}.\nIf you didn't forget your password, please ignore this email.`;

    try {
        await sendEmail({
            email: user.email,
            subject: "Your password reset token (valids for 10 minutes)",
            message
        });
...
exports.resetPassword =  catchAsync ( async (req, res, next) => {
    ...
    //4) Log the user in, send the Json web token (jwt) to the client.
    createSendToken(user, 200, res);
});
